...
-- pegar todos os dados do cliente em que um cliente tem um pedido
SELECT * FROM cliente
JOIN pedido ON cliente.cliente_id = pedido.cliente_id
where cliente.cliente_id = 1; -- filtro 

-- cliente que fez o pagamento com pix
select cliente.nome, pedido.pedido_id, forma_pagamento.tipo from cliente
join pedido on cliente.cliente_id = pedido.cliente_id
join forma_pagamento on pedido.forma_pagamento_id = forma_pagamento.forma_pagamento_id
where forma_pagamento.tipo = 'Pix';

-- todos os pedidos agrupados por tipo de forma de pagamento 
...
select item.nome, item.preco from item_pedido
join pedido on pedido.pedido_id = item_pedido.pedido_id
