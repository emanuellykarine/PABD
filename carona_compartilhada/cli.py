...
class CaronaCLI:
    ...
    def buscar_carona(self):
        """Busca uma carona por faixa de preço"""
        print("\nBUSCAR CARONA")
        print("=" * 80)

        try:
            preco = int(input("Preço desejado para a carona: "))

            resultado = self._make_request("GET", "caronas/?motorista=&status=&origem=&destino=&preco_por_pessoa={}".format(preco))

            if resultado:
                print(f"\nCaronas encontradas:")
                for carona in resultado.get('results', []):
                    print(f" - ID: {carona.get('id')}, Preço: {carona.get('preco_por_pessoa')}")
            else:
    ...
    def menu_principal(self):
        ...
        while True:
            ...
            print("7. Buscar carona por faixa de preço")
            print("0. Sair")
            print("=" * 80)
